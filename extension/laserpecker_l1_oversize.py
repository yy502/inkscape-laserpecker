...
class laser_gcode(inkex.Effect):
    ...
    def offset_gcode(self, gcode):
        # Offset X,Y coordinates for oversize engraving.
        lines = gcode.split('\n')
        ...
        if width > 100.1 or height > 100.1:
            self.error(_("Graphics are too big: %.1f x %.1f.\n\nScale it down to within 100x100 or select less objects/paths for LaserPecker to function properly." % (width, height)),"error")
            return ""
        _gcode = ""
        for line in lines:
            if line == "G1 X0 Y0" or line.startswith("G1 X0 Y0 "):
                ...
                for word in words:
                    if word.startswith('X'):
                        x = float(word[1:]) - 100  # half of 200mm
                        line = line.replace(word, "X%.5f" % x)
                    elif word.startswith('Y'):
                        y = float(word[1:]) - 70  # half of 140mm
    ...
    def export_gcode(self, gcode):
        ...
        f.write(self.header + self.offset_gcode(gcode) + self.footer)
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--correct-page-size",               action="store", type="inkbool",         dest="correct_page_size",                   default=False,                          help="Change page size to 200x140 (mm)")
...
################################################################################
    def effect(self) :
        ...
        if not self.options.correct_page_size:
            self.error(_("You must change the page size to 200x140 (mm) for this to work."),"warning")
            return
        self.get_info()
