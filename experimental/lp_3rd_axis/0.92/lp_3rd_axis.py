...
class laser_gcode(inkex.Effect):
    ...
    def transform_gcode(self, gcode):
        ...
        try:
            ...
            return ""
        if width > 100.1:
            self.error(_("Graphics are too wide: %.1f x %.1f.\n\nScale it down to within width of 100 for LaserPecker to function properly." % (width, height)),"error")
            return ""
        
        # sort and order lines by Y
        paths_by_y = {}
        for path in gcode.strip().split("\n\n"):
            y = round(float(path.strip().split("\n")[0].split("Y")[1]), 1)
            if y in paths_by_y.keys():
                paths_by_y[y].append(path)
            else:
                paths_by_y[y] = [path]
        sorted_paths = []
        for y in sorted(paths_by_y.keys(), reverse=True):
            for p in paths_by_y[y]:
                sorted_paths.append(p)
        lines = "\n\n".join(sorted_paths).split("\n")
        # offset X/Y values
        _gcode = ""
        laser_on = False
        last_y = 99999
        for line in lines:
            ...
            if line.startswith("G1 X0 Y0"):
                ...
                for word in words:
                    if word.startswith('X'):
                        ...
                        line = line.replace(word, "X%.1f" % x)
                    elif word.startswith('Y'):
                        y = (float(word[1:]) - y_min) % step_size
                        line = line.replace(word, "Y%.1f" % ((y % step_size) + y_offset))
            ...
            if line == ";laser on":
                ...
                laser_on = False
            if not laser_on and start_of_block and line.startswith("G1  X"):
                line = trigger + line
            _gcode += line + '\n'
    ...
    def export_gcode(self, gcode):
        ...
        f.write(self.header + self.transform_gcode(gcode) + self.footer)
        f.close()
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--laser-power",                     action="store", type="int",             dest="laser_power",                         default="255",                          help="1-255 (1=min, 255=max)")
