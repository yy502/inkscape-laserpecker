...
1) Install Inkscape v0.92, v1.0 or v1.1
2) Depending on your Inkscape version and operating system, copy `laserpecker.inx` and `laserpecker.py` from one of the directories in `extension/` into...
...
Click to watch video tutorial (using Inkscape 0.92, v1.0/1.1 looks slightly different):
...
4) Use the following optimal settings (using Inkscape 0.92, v1.0/1.1 is slightly different):
...
### Inkscape 1.0/1.1 users
1) Inkscape 1.0/1.1 has this amazing tool built-in! \o/
...
**_To Android users: the information below only applies up to LP app v3.69. From v3.73, the app lost access to laserpecker files directory tempororily per Google's new privacy requirement. LP will add a proper feature in the app to import Gcode files in the next update._**
